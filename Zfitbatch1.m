function Rough = Zfitbatch1 ()
%% load txt from selected folder
    curPath = pwd;
    folderName = uigetdir(curPath,'Choose folder with EIS in it.');
    cd(folderName);
    ...
    for x = 1:length(files)
        ...
        if ~isempty(strfind(name, '.txt')) || ...
                ...
                ~isempty(strfind(name, '.txt'))
            rFiles = [rFiles files(x)];  %#ok<*AGROW>
        end
    end
    % read all data
    rData = [];
    for i = 1:length(rFiles)
        rID = fopen(rFiles(i).name);
        rDat = textscan(rID, '%f%f%f%f%f', ' ');
        rData = [rData rDat(:,1:3)];
        fclose(rID);
    end
      
%% fit by zfit and calculate roughness 
%ex. [p,zm] = Zfit(zdata, 'z', 's(R1,E2)', [500, 1e-6, 0.96], [1:36],'fitNP')
Pr = [];
Zm = [];
    for i = 1:length(rFiles)
        zdata = [rData{:,3*i-2}, rData{:,3*i-1}, rData{:,3*i}];
        [p, zm] = Zfit(zdata, 'z', 's(R1,E2)', [500, 1e-6, 0.96], [1:36],'fitNP');
        Pr = [Pr; p];
        Zm = [Zm zm];
    end
%roughness equation ([R^(1-n)*Q]^(1/n))/0.0201 
    Rf = [];
    for i = 1:length(rFiles)
        Cd = (((Pr(i,1).^(1-Pr(i,3))).*Pr(i,2)).^(1./Pr(i,3)))/0.0201
        r = Cd.*50000;
        Rf = [Rf r];
    end
...
Rough = {};
    for i = 1:length(rFiles)
        rough = {rFiles(i).name, Rf(i)};
        Rough = [Rough; rough];
    end

end

% Zfit function is provided by JOHN D'ERRICO, jean-luc.dellis@u-picardie.fr
